...
RULES = {
    "nonfiction": (
        ...
        "每一处因果、每一份判断都以本篇资料块所载为据, 别篇的事实在此处以一句指代。"
    ),
    ...
    # v34 (问题5, 用户拍板「全部进」): 单篇写法 — 一主题、一时刻、一悬念。
    "narrative": (
        "「一篇一题」: 每篇先立一个主题与一个决定性时刻 (具体的年、地、动作), "
        "全篇段落皆为此题服务; 开篇从最见性情的那个时刻写起, 时间交代随事带出; "
        "每段推进新信息、新关系或新后果, 让读者一直往前走; "
        "让主角经他人的眼睛被看见——对手、下属、配偶各有一处行事可写; "
        "结尾回到开篇那个时刻, 让人物与开头相比已不是同一副模样。"
    ),
    # v34 (问题5): 各篇共用一料而不复述 —— 《史记》互见法的程序化口径。
    "intertext": (
        "「互见法」: 同一事只在最贴切的那一篇详写, 分配如下——"
        "主角公开生平归《本纪》; 传主生平归其本传; 门庭内情与子女来历归《家室列传》; "
        "隐事把柄归《阴私录》; 死者生平归《刺客列传》; 家族仇怨归《家族恩怨录》; "
        "其余各篇以一句指代即可; 全篇共用同一套称谓与年头, 篇篇各有其题而不相复述。"
    ),
}
...
SECTION_REQ = {
    ...
    "feuds": {
        "lead": "写与主角家族关系不和的各家族: 结怨之由、恩怨始末、当前关系档位 (世仇/敌对/争吵), 以资料为限。恩怨之始按战争与领地的次序写出: 宣战、胜负、领地易主、对方此后处境; 囚禁与俘虏写在对应战役的过程里。",
        "mid": "依事件史叙述各家族的恩怨始末: 联姻、囚禁、处决、宣战、反目等, 以资料为限。本篇把每段恩怨的起点 (宣战与夺地的日期与由头) 写到收束, 让恩怨链条从开战、战败、失地到对方处境完整可见。",
        "tail": None,
    },
    "artifacts": {
...
FACT_WORDING = {
    ...
    # v34 (问题7): 未见释放记载时的收句 — 程序把「记载到此为止」说全,
    # 让模型有完整句子可依, 而非留一个待补的空位 (旧稿此处留白,
    # 模型遂把「囚期记载未着一字」补成「获释」)。
    "prison_still_held": "，此后再未见释放的记载",
