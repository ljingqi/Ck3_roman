...
CK3 自动存档 (.ck3)  —(watch/continue 只处理启动后写入的新存档)
  ...
  → 记忆提取 → output/<家族>/data/player_<玩家id>.json（每玩家一份，跨年去重）
...
- 每玩家一份缓存 `output/<家族>/data/player_<id>.json`（v4 起不再用顶层 `cache/`，
  由 `migrate` 迁移）；主角死亡、继承人继位（同战役新玩家）后自动为新主角建档。
- 死亡检测：新存档中检测到前代玩家（同战役）的 dead_data 即触发；**带身份校验**
  （名字一致 + 死亡日期晚于最后存活档）。每次死亡**只生成一篇终传**。
- **家族文件夹**：`output/<家族名>\`（如 `冯·大马士革`、`边氏`）；watch 新战役
  重名自动加数字（哈布斯堡 → 哈布斯堡2），continue 自动沿用该家族最新文件夹；
...
| `data/` | melt JSON + names.json + localization.json + province_map.json |
| `output/<家族>/data/` | 每玩家记忆缓存（v4 起） |
| `output/` | 传记输出（按家族分文件夹） |
