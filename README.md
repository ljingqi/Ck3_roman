...
- **动态国名按当前时点**：死者官职地名改取「当前/as_of 时点」国号（`_last_title_place`），
  王言 886 年上任时的「关内王」按游戏显示为「秦王」（国号 关内→翟→秦）。
...
- **世系编号（II/III 二世标记）**：编号不存存档，按「首要头衔 title history 同名前任数 + 1」
  ...
  刺客列传/人物档案，鲁斯兰父子 → 鲁斯兰·克里维奇 与 鲁斯兰·克里维奇二世。
  编号位置与中文版游戏一致（模板 `$FIRST_NAME$·$REMAINING_NAME$$NUMBER$`，在名字末尾）。
- **昵称**：读熔件 `nickname_text`（直接是中文昵称），按文化名序插在名后——
  西方 名“昵称”·家名（萨洛蒙“外来者”·马布·里瓦朗）、东方 姓+名“昵称”（大和惟条“秃头”），
  与世系组合 名“昵称”·家名二世（玛拿西“义人”·布兰二世）。
